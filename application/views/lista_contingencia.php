...
<body>
	...
	<div class="container">
		...
		<div class="row center-align">
			<a class="waves-effect waves-light blue darken-1 btn-small" href="#" onclick="getcontingencia()"><i class="material-icons">refresh</i></a>
		</div>
    ...
    <script>
		...
		async function getcontingencia() {
			...
			let base_url = "<?= base_url() ?>getcontingenciabyuser";
			let options = {
				method: 'GET',
				headers: {
			...
			resdata.message.forEach(val => {
				...
				let estado = (val.finalizado == null) ? '<span class="badge grey white-text" style="border-radius: 30px;">Pendiente</span>' : '<span class="badge blue darken-4 white-text" style="border-radius: 30px;">Atendido</span>' ;
				let data = JSON.stringify(val);
				let dataencode = data.replaceAll('\"','|')

				let tipificacion = '';
				if (val.tipificacion == null && val.engestion == '0') {
					tipificacion = '';
				} else if (val.tipificacion == null && val.engestion == '1') {
					tipificacion = 'En Proceso';
				} else if (val.tipificacion == 'OK') {
					tipificacion = 'Finalizado'
				} else {
					tipificacion = 'Rechazado'
				}

				content += `<tr onclick="detallesolicitud('${dataencode}')">
						<td>${val.pedido}</td>
						<td>${val.horagestion}</td>
						<td>${tipificacion || ''}</td>
						<td>${estado}</td>
					</tr>`;
			});
		...
		async function detallesolicitud(data) {
			...
			let content = `
				<p><strong>Accion:</strong> ${dataformat.accion}</p>
				<p><strong>Pedido:</strong> ${dataformat.pedido}</p>
				<p><strong>Producto:</strong> ${dataformat.producto}</p>
				<p><strong>Tipificación:</strong> ${dataformat.tipificacion || ''}</p>
				<p><strong>Fecha Solicitud:</strong> ${dataformat.horagestion}</p>
				<p><strong>Fecha Respuesta: </strong> ${dataformat.horacontingencia || ''}</p>
				<p><strong>Observación Terreno:</strong></p>
				<p>${dataformat.observacion || ''}</p>
				<p><strong>Observación Despacho:</strong></p>
				<p>${dataformat.observContingencia || ''}</p>
			`;
