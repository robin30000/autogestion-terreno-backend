...
class Autenticacion extends CI_Controller
{
	...
	public function recoverPass()
	{
		...
		$login = trim(htmlentities($_GET['login'], ENT_QUOTES));
		$cedula = trim(htmlentities($_GET['cedula'], ENT_QUOTES));
		$celular = trim(htmlentities($_GET['celular'], ENT_QUOTES));

		$response = $this->Modeloautenticacion->recoverPass($login, $cedula, $celular);


		if ($response) {
			$arrayResult = ['type' => 'success', 'message' => 'Su contraseña de ingreso es: ' . $response['pass_apk']];
		} else {
			$arrayResult = ['type' => 'error', 'message' => 'Los datos ingresados no se encuentran registrados'];
		}
	...
	{
		$reqjson = json_decode($this->input->raw_input_stream, true);
		$pass = trim(htmlentities($reqjson['pass'], ENT_QUOTES));
		$confirmPass = trim(htmlentities($reqjson['confirmPass'], ENT_QUOTES));
		$cedula = trim(htmlentities($reqjson['cedula'], ENT_QUOTES));

/*		$arrayResult = ['type' => 'success', 'message' => 'Pepino '. $pass.' '. $confirmPass.' '.$cedula];
		echo json_encode($arrayResult);   Colombia2048--++
		die();*/

		if ($pass !== $confirmPass) {
			$arrayResult = ['type' => 'error', 'message' => 'El campo Nueva contraseña y Confirmar contraseña no son iguales'];
		} elseif (strlen($pass) < 6 || !preg_match('/^(?=.*[a-z])(?=.*[A-Z]).{6,}$/', $pass)) {
			$arrayResult = ['type' => 'error', 'message' => 'La nueva contraseña debe tener al menos 6 caracteres y contener al menos una letra mayúscula y una letra minúscula.'];
		} else {
			$response = $this->Modeloautenticacion->updatePass($pass, $cedula);
			if ($response) {
				$arrayResult = ['type' => 'success', 'message' => 'La contraseña se actualizo correctamente'];
			} else {
				$arrayResult = ['type' => 'error', 'message' => 'Ha ocurrido un error interno intentalo nuevamente en unos minutos'];
			}
		}
		echo json_encode($arrayResult);
		die();
